...
//Declare the height and width of the boundary
const int width = 80;
const int height = 20;
...
void GameRender(string playerName)
{
	...
	//Function for the updating game stats
void Updategame()
{
	...
	//Checks for snake's collisions with the wall (|)
	if (x >= width || x < 0 || y >= height || y > 0)
		isGameOver = true;

	//Checks for snake's collisions with the tail (o)
	for (int i = 0; i < snakeTaillen; i++) {
		if (snakeTailX[i] == x && snakeTailY[i] == y)
			isGameOver = true;
	}

	//Checks for snake's collisions with the food (#)
